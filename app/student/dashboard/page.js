...
import { getStudentComplaints, createComplaint, demoStudents } from "../../../lib/db";

export default function StudentDashboard() {
  const [student, setStudent] = useState(null);
  const [loading, setLoading] = useState(true);
  const [complaints, setComplaints] = useState([]);
  const [activeTab, setActiveTab] = useState("overview"); // 'overview' | 'complaints' | 'rent' | 'mess'
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("pg_student_user");
      if (stored) {
        try {
          const parsed = JSON.parse(stored);
          const fullData = demoStudents.find(s => s.email === parsed.email || s.phone === parsed.phone) || {
            ...demoStudents[0],
            ...parsed
          };
          setStudent(fullData);
          ...
          setStudent(demoStudents[0]);
          loadComplaints(demoStudents[0].phone);
        }
      } else {
        // Default to first demo student for preview if direct navigation
        setStudent(demoStudents[0]);
        loadComplaints(demoStudents[0].phone);
      }
      setLoading(false);
  ...
  const loadComplaints = async (phone) => {
    const list = await getStudentComplaints(phone || "+91 9988776655");
    setComplaints(list);
  };
  ...
  const handleCreateComplaint = async (e) => {
    ...
    const newComp = {
      pg_id: student?.pg_id || "3",
      room_number: student?.room_number || "204",
      student_name: student?.name || "Student",
      student_phone: student?.phone || "+91 9988776655",
    ...
    if (!res.error) {
      setComplaintSuccess("Complaint submitted successfully! Warden & Admin notified.");
      setNewComplaintDesc("");
      setShowComplaintModal(false);
      loadComplaints(student?.phone);
  ...
  const copyWifiPassword = () => {
    if (student?.wifi_pass && navigator.clipboard) {
      navigator.clipboard.writeText(student.wifi_pass);
      setCopiedWifi(true);
      ...
      <div className="bg-slate-900/90 border-b border-slate-800 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="bg-purple-950 border border-purple-800/60 text-purple-300 text-xs font-bold px-2.5 py-1 rounded-lg">
              🎓 Student Portal
            </span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-purple-300 font-medium text-sm sm:text-base">
                  {student.pg_name} • Room <span className="font-bold text-white">{student.room_number}</span>
                </p>
                <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400 mt-2">
                  <span>📱 {student.phone}</span>
                  <span>✉️ {student.email}</span>
                  <span>📅 Move-in: {student.move_in_date || "Jan 2024"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/80 backdrop-blur-md border border-slate-800 p-4 rounded-2xl flex flex-row md:flex-col justify-between items-center md:items-end gap-2 flex-shrink-0">
              <div className="text-left md:text-right">
                ...
                <span className="text-lg font-bold text-emerald-400">{student.rent_status || "Paid"} ({student.rent_amount})</span>
              </div>
              <span className="text-xs text-slate-400">Next due: {student.rent_due_date || "05th Sep 2026"}</span>
            </div>
          </div>
        </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden">
              ...
              <div className="space-y-3 bg-slate-950 p-4 rounded-2xl border border-slate-800/80 mb-4">
                <div>
                  ...
                  <span className="text-sm font-mono font-bold text-purple-300">{student.wifi_ssid || "DreamHomes_Civil_204"}</span>
                </div>
                <div>
                  <span className="text-xs text-slate-400 block uppercase font-semibold">Password</span>
                  <span className="text-sm font-mono font-bold text-white">{student.wifi_pass || "DH#Civil2026"}</span>
                </div>
              </div>
              <button
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-slate-950 rounded-2xl border border-slate-800">
                  <div>
                    ...
                    <span className="text-sm font-bold text-white">{student.warden_name || "Ramesh Sharma"}</span>
                  </div>
                  <a
                    href={`tel:${student.warden_phone || "+919876511223"}`}
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  <tr>
                    ...
                    <td className="py-4 px-4 text-slate-200">{student.rent_amount || "₹8,500"}</td>
      ...
      {showComplaintModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl relative animate-fadeIn">
            ...
            <p className="text-slate-400 text-xs mb-6">Room {student.room_number} • {student.pg_name}</p>
